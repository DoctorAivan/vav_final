...
//  Actualizar el tiempo y estilo de la ultima modificación
	function mesas_ultima_actualizacion_switch()
	{
		mesas_ultima_actualizacion_intervalo += 1
	...
	//	Selecciona todos los elementos con la clase "timeago"
		...
		//	Determinar el texto de "hace X" según el tiempo transcurrido
			...
			if (seconds < 60) {
				...
				newText = `Hace ${minutes} minuto${minutes !== 1 ? 's' : ''} y ${seconds - 60} segundo${seconds !== 1 ? 's' : ''}`;
			} else if (minutes < 60) {
				newText = `Hace ${minutes} minuto${minutes !== 1 ? 's' : ''}`;
			} else if (hours < 24) {
				newText = `Hace ${hours} hora${hours !== 1 ? 's' : ''}`;
...
//	-		-		-		-		-		-		-		-		-		-		-		-		-		-		-		-
	...
	//	Actualizar el orden de las mesas
	function actualizarOrdenTimerSwitch()
	{
	...
	//	Ordenar los elementos usando la función 'actualizarOrdenMesasSwitch'
		elements.sort(actualizarOrdenMesasSwitch);
	...
	function actualizarOrdenMesasSwitch(a, b)
	{
	//	Cambiar el estado de color
		var aclass = a.className.match(estadosMesasRexSwitch);
		var avalue = aclass ? estadosMesasSwitch[aclass[0]] : 99;
		var bclass = b.className.match(estadosMesasRexSwitch);
		var bvalue = bclass ? estadosMesasSwitch[bclass[0]] : 99;
	...
	const estadosMesasRexSwitch = /(rojo|amarillo|verde)/;
